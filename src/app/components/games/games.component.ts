...
export class GamesComponent implements OnInit, OnDestroy {
    ...
    easy() {
        ...
        this.games.push({
            ...
            // Store it for the next promise step, firebase doesn't pass it after .set()
            this.createdGame = game.key;
            ...
            return this.ngFire.database.object('boards'.concat('/', this.auth.uid, '/', game.key))
                .set(this.boardService.generate(9, 9));
        }).then(() => {
            // After creating the game, open it
            this.ngFire.database.object('games'.concat('/', this.auth.uid, '/', this.createdGame))
                .forEach(game => {
                    this.selectedGame.next(game);
                });
        });
